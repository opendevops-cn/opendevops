...
**确认数据库信息**
初始化数据库：codo_cron, 请登陆确认是否有初始化数据，没有请手动导入data.sql数据
```
...
```
CRON_DB_DBNAME='codo_cron' #后端数据库名称,建议不要修改，初始化data.sql已经指定了数据库名字，若需改请一块修改
sed -i  "s#server_name .*#server_name ${task_domain};#g" doc/nginx_ops.conf #定时任务域名
